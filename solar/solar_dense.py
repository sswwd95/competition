...
def Model():
    model = Sequential()
    model.add(Dense(512,activation='relu', input_shape = (1,8)))
...
for q in quantiles:
    model = Model()
    modelpath = '../solar/check/solar_0121_dense_day7_{epoch:02d}_{val_loss:.4f}.hdf5'
    cp = ModelCheckpoint(filepath=modelpath, monitor='val_loss', save_best_only=True, mode='auto')
...
for q in quantiles:
    model = Model()
    modelpath = '../solar/check/solar_0121_dense_day8_{epoch:02d}_{val_loss:.4f}.hdf5'
    cp = ModelCheckpoint(filepath=modelpath, monitor='val_loss', save_best_only=True, mode='auto')
    model.compile(loss=lambda y_true,y_pred: quantile_loss(q,y_true, y_pred), optimizer='adam')
...
sub.to_csv('./solar/csv/sub_0121_dense.csv', index=False)
